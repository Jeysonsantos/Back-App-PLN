...
def dif_algumas_tags(tag):
    ...
    string_tag=str(tag)
    retorno = True
    lista_retirar_tags = ['<a ','<p>','<h1','<im','<ul','<li','<sp','<sv','<so','<pi','<pa']
...
def num_p(pagina):
    ...
    for tag_1 in pagina:
        if is_tag(tag_1) and dif_algumas_tags(tag_1):
            ...
            quantidade_paragrafos_por_tag[tag_completa]=quant_paragraph(tag_1)
            num_p(tag_1)
...
def quant_p_geral_por_tag(pagina):
    ...
    for i in pagina:
        if is_tag(i) and dif_algumas_tags(i):
            quantidade_paragrafos_por_tag={}
            quantidade_paragrafos_por_tag=num_p(i) # IDENTIFICAR POR QUE O i NAO ES´TA FUNCIONANDO DA MANEIRA QUE DEVERIA
